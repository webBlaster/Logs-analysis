...
first_sql = """select replace(log.path, '/article/', ''),
            count(*) as num from log, articles 
...
second_sql = """ select authors.name,count(*) as num from log, articles, 
            authors where log.status = '200 OK' and 
